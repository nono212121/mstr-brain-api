Hier ist der komplette Inhalt — alles ab der ersten """ bis zur letzten Zeile kopieren und in GitHub einfügen:
python"""
...
def wb_login():
    if not WB_PASSWORD:
        return None
    if WB_PHONE:
        account, account_type = WB_PHONE, "1"
    elif WB_EMAIL:
        account, account_type = WB_EMAIL, "2"
    else:
    ...
    try:
        ...
        if not token:
            r2 = req.post("https://userapi.webull.com/api/passport/login/account",
                          json=payload, headers=WEBULL_HEADERS, timeout=15)
            d2 = r2.json()
            token = d2.get("accessToken") or d2.get("data", {}).get("accessToken")
        if token:
            _wb_session["token"] = token
            _wb_session["expires"] = time.time() + 3600 * 6
            print(f"Webull Login erfolgreich ({'Telefon' if account_type=='1' else 'Email'})")
            return token
...
def wb_get_quote(ticker_id):
    try:
        ...
        d = req.get(url, headers=WEBULL_HEADERS, timeout=10).json()
        price = float(d.get("close") or d.get("pPrice") or 0)
        pre   = float(d.get("preClose") or 0)
        return {"price": round(price, 2), "change_pct": round((price-pre)/pre*100, 2) if pre else 0}
    except Exception as e:
...
def wb_get_options(ticker_id, spot):
    try:
        ...
        expire_list = req.get(url, headers=WEBULL_HEADERS, timeout=10).json()
        if not expire_list:
            return None
        exp_dates = [e.get("date", "") for e in expire_list if e.get("date")]
        best_exp  = find_best_expiry_days(exp_dates, 42) or (exp_dates[0] if exp_dates else None)
        if not best_exp:
            return None
        ...
        chain_data = req.get(chain_url, headers=WEBULL_HEADERS, timeout=12).json()
        if not chain_data:
            return None
        ...
        for opt in options_list:
            try:
                ...
                iv     = float(opt.get("impVol", 0) or 0) or 0.88
                dr     = opt.get("delta")
                delta  = round(abs(float(dr)), 3) if dr is not None else round(bs_delta(spot, strike, T, iv), 3)
                if delta < 0.03 or delta > 0.22:
                    continue
                rows.append({"strike": strike, "delta": delta,
                             "bid": round(bid,2) if bid else None,
                             "ask": round(ask,2) if ask else None,
                             "mid": mid,
                             "iv": round(iv*100,1) if iv < 5 else round(iv,1),
                             "dte": dte, "otm_pct": round((strike-spot)/spot*100,1),
                             "volume": int(opt.get("volume",0) or 0),
                             "open_interest": int(opt.get("openInterest",0) or 0)})
            except:
                continue
        rows.sort(key=lambda x: x["delta"], reverse=True)
        return {"expiry": best_exp, "dte": dte, "spot": round(spot,2), "source": "webull", "options": rows}
    except Exception as e:
...
def index():
    tid = get_ticker_id()
    return jsonify({"status": "MSTR Brain API v2.3 ✅",
...
def get_mstr():
    def fetch():
        tid = get_ticker_id()
        if not tid: return {"error": "Ticker ID nicht gefunden"}
        q = wb_get_quote(tid)
        if not q: return {"error": "Quote fehlgeschlagen"}
        return {**q, "delayed": True, "as_of": datetime.utcnow().isoformat()+"Z"}
    try: return jsonify(cached('mstr', fetch))
...
def get_btc():
    ...
    try: return jsonify(cached('btc', fetch))
    except Exception as e: return jsonify({"error": str(e)}), 500
...
def get_fg():
    def fetch():
        ...
        return {"value": int(data[0]["value"]), "label": data[0]["value_classification"],
                ...
                "as_of": datetime.utcnow().isoformat()+"Z"}
    try: return jsonify(cached('fg', fetch))
    except Exception as e: return jsonify({"error": str(e)}), 500
...
def get_options():
    def fetch():
        ...
        res  = wb_get_options(tid, spot)
        if not res: return {"error": "Options fehlgeschlagen"}
        res["as_of"] = datetime.utcnow().isoformat()+"Z"
        return res
    try: return jsonify(cached('options', fetch))
    except Exception as e: return jsonify({"error": str(e)}), 500

@app.route('/all')
def get_all():
    def fetch_all():
        out = {}; spot = 150.0; tid = get_ticker_id()
        try:
            q = wb_get_quote(tid) if tid else None
            if q: spot = q["price"]; out['mstr'] = {**q, "delayed": True}
            else: out['mstr'] = {"error": "fehlgeschlagen"}
        except Exception as e: out['mstr'] = {"error": str(e)}
        try:
            r = req.get("https://api.coingecko.com/api/v3/simple/price",
                params={"ids":"bitcoin","vs_currencies":"usd",
                        "include_24hr_change":"true","include_7d_change":"true"}, timeout=8)
        ...
        except Exception as e: out['btc'] = {"error": str(e)}
        try:
        ...
        except Exception as e: out['fg'] = {"error": str(e)}
        try:
            opts = wb_get_options(tid, spot) if tid else None
            out['options'] = opts or {"error": "Keine Daten"}
        except Exception as e: out['options'] = {"error": str(e)}
        out['as_of'] = datetime.utcnow().isoformat()+"Z"
        return out
    try: return jsonify(cached('all', fetch_all))
    except Exception as e: return jsonify({"error": str(e)}), 500
...
def send_morning_briefing():
    ...
    try:
        tid = get_ticker_id(); mstr=0; mchg=0
        if tid:
            q = wb_get_quote(tid)
            if q: mstr=q["price"]; mchg=q["change_pct"]
        r    = req.get("https://api.coingecko.com/api/v3/simple/price",
